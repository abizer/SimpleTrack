...
def insert_issue(description):
	...
	g.db.execute(stmt, {'description': description, 'status': 'open', 'date': date.today()})
	g.db.commit()
...
@app.route('/')
def index():
	cssmap = {'open': 'bg-danger', 'pending': 'bg-warning', 'resolved': 'bg-success'} # don't like this
...
@app.route('/<status>/<int:rowid>', methods = ['POST']) # false sense of security in only allowing POST
